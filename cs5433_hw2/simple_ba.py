...
class SimplePKIBA:
    ...
    def __init__(self, sender):
        ...
        self.signatures = {} # maps proposed messages to list of accepted (node_id, signature) tuples, corresponding 
        # to valid votes
        self.sender = sender
    ...
    def calculate_votes_for(self, round):
        ...
        proposals_to_vote_for = []
        all_proposals = self.get_proposals_with_threshold(round) # all proposals that have acheived threshold


        # by definition proposal is new proposal that has just achieved threshold 
        # so we go over all the proposals we obtained using the function below

        for mp in all_proposals:

            # we need to ensure that the proposal in not already in s_i
            if mp not in self.s_i:

                # for the new proposals, j adds them to Sj
                self.votes[mp].add(config.node_id)
                self.s_i.append(mp)

                # and adds signature
                if mp in self.signatures:
                    self.signatures[mp].append((config.node_id, "a"))
                else:
                    self.signatures[mp] = [(config.node_id, "a")]

                proposals_to_vote_for.append(mp)
                # print("mp:",mp)
        return proposals_to_vote_for
    ...
    def get_proposals_with_threshold(self, round): 
        """ Gets proposals that have reached the threshold required by a given round.
            ...
            This function *DOES NOT* need to check signatures; 
            assume they are already checked in process_vote.
        """
        # Pseudocode: with signatures from r different players, including player s

        all_proposals_with_r_votes = []

        # ensuring that proposals reach the threshold required by a given round
        for mp in self.votes.keys():
            # print("mp:", mp, self.votes[mp], "round:", round, "len(self.votes[mp]):", len(self.votes[mp]))
            if round<=len(self.votes[mp]) and len(self.votes[mp])!=0 and (self.sender in self.votes[mp]):
                all_proposals_with_r_votes.append(mp)
            elif round<=len(self.votes[mp]) and len(self.votes[mp])==0 and (self.sender in self.votes[mp]):
                continue

        return all_proposals_with_r_votes
    ...
    def get_output(self):
        ...
        # Pseudocode: (Output) Each player i outputs 0 if |Si| > 1 
        # and otherwise outputs the unique element in Si.
        # placeholder for (3.3)

        # need to ensure that the protocol is complete
        if self.is_done() == True:

            # outputs unique element
            if len(self.s_i)==1:
                return self.s_i[0]
            elif len(self.s_i)>1 or len(self.s_i)==0:
                return 0

        # if protocol is not complete, return None
        return None
