...
def gossip_message(type, message):
    ...
    for key, address in config.PEERS.items():
        if key != config.node_id:
            send_message( address ,type, message)
    ...
    #         expected_output.add((config.PEERS[i+1], type, message))
    # else:
    #     for i in range(len(config.PEERS)):
    #         keys.append(i+1)
    #     keys.remove(config.node_id)
    #     for key in keys:
    #         expected_output.add((config.PEERS[key], type, message))
